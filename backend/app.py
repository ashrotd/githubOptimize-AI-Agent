...
def test_github():
    "Test github api working well if it can fetch user profiles"
    from utils.github_client import GithubClient

    client = analyzer.analyze_profile("ashrotd")
    #profile = client.get_user_profile('ashrotd')
    #
    
    #repos = client.get_user_repos('ashrotd')
    return jsonify({
        "success": client
    }), 200
